...
if(isset($_POST['submit']) && !$errors) {
	...
	$recipeSql="UPDATE recipes SET title = '". $title ."', description = '". $description ."', serves = '". $serves ."', preparationtime = '". $preparationtime ."', cookingtime = '". $cookingtime ."'	WHERE id = ". $recipeid;
	mysql_query($recipeSql);
		
	// insert the image if we have one
	if ($image){
		// delete the old one first!	
		$imageDelete = "DELETE FROM images WHERE recipeid =". $recipeid;
		mysql_query($imageDelete);
		$imageSql="INSERT INTO images (recipeid, imagelocation) VALUES ($recipeid, '$image_name')";       
		mysql_query($imageSql);
	}

	// loop through the ingredients and run a sql insert for each
	// delete the old ones first!
	$ingredientsDelete = "DELETE FROM ingredients WHERE recipeid =". $recipeid;
	mysql_query($ingredientsDelete);
	foreach ($_POST['ingredient'] as $key=>$value) {
		$ingredientSql="INSERT INTO ingredients (recipeid, ingredient) VALUES ($recipeid, '$value')";
		mysql_query($ingredientSql);
	}

	// loop through the preparation steps and run a sql insert for each
	// delete the old ones first!
	$preparationDelete = "DELETE FROM preparation WHERE recipeid =". $recipeid;
	mysql_query($preparationDelete);
	foreach ($_POST['preparation'] as $key=>$value) {
		$preparationSql="INSERT INTO preparation (recipeid, preparation) VALUES ($recipeid, '$value')";
		mysql_query($preparationSql);
	}
	
	// delete the old tags first!
	$tagsDelete = "DELETE FROM recipe_tags WHERE recipeid =". $recipeid;	
	mysql_query($tagsDelete);
	
	// loop through the ready in and run a sql insert for each
	if (isset($_POST['readyIn'])) {
		foreach ($_POST['readyIn'] as $key=>$value) {
			$readyInSql="INSERT INTO recipe_tags (tag_id, recipeid) VALUES ('$value', $recipeid)";
	...
	if (isset($_POST['courses'])) {
		foreach ($_POST['courses'] as $key=>$value) {
			$coursesSql="INSERT INTO recipe_tags (tag_id, recipeid) VALUES ('$value', $recipeid)";
			mysql_query($coursesSql);
		}
	}
	// loop through the special diet and run a sql insert for each
	if (isset($_POST['specialDiet'])) {
		foreach ($_POST['specialDiet'] as $key=>$value) {
			$specialDietSql="INSERT INTO recipe_tags (tag_id, recipeid) VALUES ('$value', $recipeid)";
	...
	if (isset($_POST['cuisine'])) {
		foreach ($_POST['cuisine'] as $key=>$value) {
			$cuisineSql="INSERT INTO recipe_tags (tag_id, recipeid) VALUES ('$value', $recipeid)";
			mysql_query($cuisineSql);
		}	
	}
	
	// take the user to the page where their recipe has just been added
	header( 'Location: /recipes.php?recipeid='.$recipeid ) ;
